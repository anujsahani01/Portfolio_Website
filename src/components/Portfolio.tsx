...
import { ChevronDown, ChevronUp, ExternalLink, Play, FileText, Bot, Database, Code, Activity, Eye } from "lucide-react";
const Portfolio = () => {
  ...
  const projects = [{
    ...
    links: [{
      ...
      url: "https://vimeo.com/1130637233?share=copy&fl=sv&fe=ci"
    }]
  }, {
    id: 3,
  ...
  return <section id="portfolio" className="bg-gradient-to-b from-secondary/20 to-background">
      <div className="section-padding">
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          {projects.map((project, index) => <Card key={project.id} className="card-hover border-border/50 bg-card/50 backdrop-blur-sm group overflow-hidden" style={{
              ...
              <CardHeader className="cursor-pointer" onClick={() => toggleCard(project.id)}>
                <div className="flex items-start justify-between">
                  ...
                  <Button variant="ghost" size="sm" className="shrink-0">
                    {expandedCard === project.id ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                  </Button>
                </div>
              </CardHeader>

              {expandedCard === project.id && <CardContent className="pt-0">
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {project.techStack.map(tech => <Badge key={tech} variant="secondary" className="text-xs px-2 py-1 hover:bg-accent hover:text-accent-foreground transition-colors duration-300">
                            {tech}
                          </Badge>)}
                      </div>
                    </div>
        ...
        <div className="mt-16">
          <Card className="max-w-4xl mx-auto border-accent/20 bg-gradient-to-r from-accent/5 to-hero-accent/5 backdrop-blur-sm">
            <CardContent className="p-8">
              ...
              <p className="text-lg text-muted-foreground leading-relaxed mb-4">Designed and built an AI-powered code assistant that can instantly search, understand, and provide relevant answers from a company’s entire codebase. The system works on two levels — a global library containing all past projects and frameworks, and smaller, team or user-specific libraries for current work. I created a smart way to break down and store code so each piece has all the information it needs for accurate answers, then indexed it in a high-speed search database for quick retrieval. The system updates automatically whenever new code is saved or committed, ensuring the assistant always works with the latest information, and I’m now enhancing it for even faster and more precise responses.</p>
              <div className="flex flex-wrap gap-2">
                {["Multi-tier RAG", "Tree-sitter", "FAISS", "GraphQL", "Event-driven indexing"].map(tech => <Badge key={tech} variant="secondary" className="text-xs px-2 py-1">
                    {tech}
                  </Badge>)}
              </div>
            </CardContent>
